...
`atlas_backend.personality`.

**Integration note (added when it was wired in).** It now runs at the end of
every assistant turn, after the audit entry, and is configured by
`ATLAS_PERSONALITY_ENABLED`, `ATLAS_PERSONALITY_MODE` and
`ATLAS_PERSONALITY_ADDRESS`. One behaviour changed during integration: the
address moved from the front of the reply to the end. "Сэр, Свободно 42 ГБ." is
wrong Russian — a capital after a comma — and the next word may be "VS Code" or
"Chrome", so there is no safe way to lowercase it. Every proactive notification
in this system already ends "..., сэр", and two halves of one assistant
addressing the owner in two different shapes is a seam a person notices without
