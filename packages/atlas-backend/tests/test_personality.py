...
def test_default_is_jarvis_but_address_is_not_repeated():
    ...
    first = render(turn)
    assert first.text == "Сэр, Свободно 42 ГБ."
...
def test_address_and_personal_mode_settings():
    ...
    assert render(turn, mode=Mode.CUSTOM, address=Address.SIR).text.startswith("Sir, ")
...
def test_untrusted_words_are_not_settings_or_permissions():
    ...
    assert render(turn).text == "Sir, " + text
